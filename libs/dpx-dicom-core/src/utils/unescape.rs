...
impl Unescaper {
	...
	pub fn unescape(&mut self) -> Result<String> {
		...
		let offset = |mut e, remaining_count| {
			let (Error::IncompleteStr{pos} | Error::InvalidChar { pos, .. } | Error::ParseInt { pos, .. }) = &mut e;
		...
		while let Some(c) = self.chars.pop() {
			...
			let c = self.chars.pop().context(IncompleteStrSnafu{ pos : chars_count - self.chars.len() - 1} )?;
			let c = match c {
				'b' => '\u{0008}',
				'f' => '\u{000c}',
				'n' => '\n',
	...
	fn unescape_unicode_internal(&mut self) -> Result<char> {
		let c = self.chars.pop().context(IncompleteStrSnafu{pos: 0usize})?;
		...
		.context(InvalidCharSnafu{char: unicode.chars().last().unwrap(), pos: 0usize})
	}
	...
	fn unescape_octal_internal(&mut self, c: char) -> Result<char> {
		...
		match c {
			...
			_ => InvalidCharSnafu { char: c, pos: 0usize }.fail()?,
		}
